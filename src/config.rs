...
/// link in a Discord message is resolved to its episode audio, transcribed
/// (preferring the publisher's RSS transcript, falling back to ffmpeg-chunked
/// STT), and the transcript plus a summary instruction is injected as text for
/// the downstream agent. Transcription reuses the `[stt]` provider config.
#[derive(Debug, Clone, Deserialize)]
pub struct PodcastConfig {
    #[serde(default)]
    pub enabled: bool,
    /// Cap on total audio minutes sent to STT when falling back to chunked
    /// transcription (guards against runaway cost on very long episodes).
    #[serde(default = "default_podcast_max_minutes")]
    pub max_minutes: u32,
    /// Instruction prepended to the transcript that tells the agent how to
...
fn default_podcast_max_minutes() -> u32 { 90 }
